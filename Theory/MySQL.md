...
| 核心功能 | 主从复制                             | 崩溃恢复                                                     | 事务回滚                                                  |
| 存储文件 | 无限追加                             | 循环写                                                       | 回收写                                                    |
| 伪示例   | `UPDATE user SET age=20 WHERE id=1;` | `lsn=123456, space_id=5, page_no=120, offset=0x60, old=18, new=20` | `trx_id=1009, table=user, row_id=1, before image: age=18` |
...
```sql
CREATE INDEX idx_name ON user(age, name);
```
...
SELECT * FROM user WHERE age = 1 AND name = 'M%';
```
...
- 索引字段被包裹在函数里面，或者参与了运算
    ...
    ```sql
    SELECT * FROM user WHERE YEAR(birth) = 1990;
    SELECT * FROM user WHERE age + 1 >= 22;
    ```

- 字符串的模糊查询不是前缀匹配，即以 % 开头

    ```sql
    SELECT * FROM user WHERE name LIKE '%dasi';
    ```
...
- 为高选择性的列建立索引，因为选择性 = 不同值数量 / 总记录数，选择性越高索引过滤效果越好
- 为查询频繁的列建立索引，不是 SELECT 的列，而是经常出现在 WHERE、JOIN、ORDER BY、GROUP BY 中的列
- 覆盖索引优先，因为覆盖索引指的是索引列本身就是需要 SELECT 的列，不需要回表

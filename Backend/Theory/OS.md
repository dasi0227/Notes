...
<img src="./attachments/image/20251001_1945701.png" alt="image-20251001194553633" style="zoom:50%;" />
...
<img src="./attachments/image/20251003_0012666.png" alt="image-20251003001235002" style="zoom:50%;" />
...
![image-20251004150556006](./attachments/image/20251004_1505084.png)
...
<img src="./attachments/image/20251004_1529784.png" alt="image-20251004152907703" style="zoom:50%;" />

#### 连续内存分配

固定分区：把内存提前划分为若干固定大小的分区，每个进程固定装入一个分区，没有外部碎片但是内部碎片很严重

![image-20251004155248028](./attachments/image/20251004_1552091.png)
...
![image-20251004154614410](./attachments/image/20251004_1546482.png)

#### 非连续内存分配

段式管理：把程序按逻辑单位（如代码段、数据段、栈段、库、堆等）划分为若干可变长度的段，每个段有基址和长度，地址由 **<段号, 段内偏移>** 表示

<img src="./attachments/image/20251004_1550765.png" alt="image-20251004155035693" style="zoom:50%;" />

页式管理：把虚拟地址空间和物理地址空间按相同固定大小的页划分，地址由 **<页号, 页内偏移>** 表示

![image-20251004155050112](./attachments/image/20251004_1550180.png)
...
<img src="./attachments/image/20251004_1620883.png" alt="image-20251004162030794" style="zoom:50%;" />
...
页表（Page Table）：是操作系统为每个进程维护的一个数据结构，位于内存中，保存了虚拟页号到物理页号到映射关系，页表的每一项是在运行中动态填充的，不是预先分配好的

页表项（Page Table Entry, PTE）：是页表中的一个条目，除了保存虚拟页号映射的物理页号，还保存了若干元信息，通常占 4B
...
页号（Page Number）：实际上并不是额外分配的编号，而是虚拟地址或物理地址中的高位部分，用来定位页，因为逻辑地址在进程中唯一，物理地址在系统中唯一，所以可以确保页号也是唯一的

#### 分页机制

字节编址：计算机的内存以字节作为最小的访问单位，也就是说每一个地址值对应内存中的一个字节单元，而每一个字节单元可以存储 8 位信息，因此编程中也规定数据类型都是以字节为单位，比如 char=1 字节、int=4 字节、double=8 字节

小端存值：低位字节放在低地址，并且低地址作为数据的基址

页：虽然内存是按字节编址，但操作系统在分配和管理时不会以字节为粒度，而是把虚拟内存空间和物理内存空间都划分为**固定大小**的页，通常为 4 KB

内存地址：无论是虚拟地址还是物理地址，都会被划分为两部分

- 高 20 位：作为页号
- 低 12 位：作为页内数据的字节偏移

<img src="./attachments/image/20251004_1727080.png" alt="image-20251004172745013" style="zoom:50%;" />

#### 二级页表

因为每个进程都要有自己的页表，导致内存的绝大部分空间都被页表占据了，而且进程中的大多数页是用不到的，即页表大部分空间会被浪费掉

- **页表大小** = 页大小 = 4KB（页表只是一个数据结构，它依旧是分页存储的，不能混淆概念）
- **虚拟页数** = 虚拟地址空间 / 页大小 = 4GB / 4KB = 2^32 / 2^12 = 2^20 = 1048576 页
- **页表空间** = 虚拟页数 * 页表项大小 = 2^20 * 4B = 4MB
- **页表数量** = 页表空间 / 页表大小 = 4MB / 4 KB = 2^10 = 1024 个

为了避免浪费空间，完全不需要把 1024 个页表都放入内存，实际上每个页表也是存储在一页中，可以再创建一个页目录，具有 1024 个条目，每个条目指向页表。其中**页目录表是在创建进程的时候就已经分配好的，而只有当某段虚拟地址空间被真正访问时，对应的页表才会被创建并加载进内存**，从而大幅减少了页表占用的空间

- 一个页目录项也占 4B，1024 个条目正好占 4KB，页目录也正好占一页
- 前 10 位作为页目录号，中间 10 位作为页表号，后 12 位作为页内偏移量
- 对于单页访问：单级页表需要 4MB，二级页表只需要 4KB + 4KB = 8KB

![image-20251004222300606](./attachments/image/20251004_2223686.png)

### 缺页管理

#### 缺页错误

缺页错误（Page Fault）：虚拟地址允许每个进程都“好像”有 4GB 虚拟内存 / 32 位地址空间，但实际上还是受限于物理内存，只有部分页会被真正加载到物理内存之中。当软件试图访问已映射在虚拟地址空间中，但是目前并未被加载在物理内存中的一个页时，即 **MMU 无法在页表中找到有效映射时，会主动发出中断，向 CPU 报告异常，触发内核的缺页异常处理程序**

- 硬性页缺失：缺页的页表项标记为不存在，且该页不在内存中，需要从磁盘重新加载
- 软性页缺失：缺页的页表项标记为不存在，但该页仍在内存中，需要从内存重新建立映射关系
- 无效页缺失：虚拟地址无效或越界，找不到缺页的页表项

<img src="./attachments/image/20251009_0935225.png" alt="image-20251009093534589" style="zoom:50%;" />

#### 页面置换算法

当发生缺页错误时，必须确保当前请求的页可以立马访问，而之前请求的页不需要保证可以访问，因此可以换出一个旧页，腾出空间来装新的页。

局部性原理是所有置换算法效果的理论基础，页面置换算法应该尽可能满足局部性原理，从而提高命中率，降低缺失率

- **时间局部性**：近期被访问的页很可能马上再被访问
- **空间局部性**：访问某页时常会访问相邻页
- **命中率**：访问时在内存中可找到所需页的比例
- **缺失率**：访问时在内存中未找到所需页的比例

| **页面置换算法** | **淘汰** | **优点** | **缺点** |
| --- | --- | --- | --- |
| **OPT, Optimal** | 将来最长时间内不会被访问的页 | 最优 | 需要预测未来信息，不可能完全实现，是理想算法 |
| **FIFO, First In First Out** | 最先进入内存的页 | 最简单 | Belady 现象：没有遵循局部性原理，可能淘汰热点页 |
| **LRU, Least Recently Used** | 最近最久没有被访问的页 | 符合时间局部性 | 需要维护时间戳 |
| **LFU, Least Frequently Used** | 最近最少没有被访问的页 | 对长期热点建模好 | 需要维护计数器 |
| **Clock** | 环形扫描，若访问位=1 则置零并跳过，直到找到访问位=0 的页 | 近似 LRU，实现与开销折中 | 没有完全遵循时间局部性 |

#### TLB

转址旁路缓存（Translation Lookaside Buffer, TLB）：又称为快表，是 CPU/MMU 内部的一个小而快的缓存，专门用来缓存虚拟页号到物理页号的页表项。
...
<img src="./attachments/image/20251009_0947084.png" alt="image-20251009094742979" style="zoom:50%;" />
...
文件系统是操作系统在持久存储设备（磁盘、SSD、闪存）上**组织、存储和访问文件**的子系统。它把原始块设备**抽象为具有内容和元数据的文件**，并提供读写、命名、权限、空间管理、一致性与恢复等服务

- 存储管理：为文件分配物理块，维护空闲块结构，负责碎片控制与空间回收
- 文件管理：实现文件的创建、删除、读写、重命名、链接等操作，并维护文件大小、时间戳等元数据
- 权限管理：管理不同用户/进程对文件的访问权限，以保证文件的安全性和保密性

| **对比** | **普通文件 File** | **目录 Directory** |
| --- | --- | --- |
| **本质** | 保存用户数据的容器 | 保存目录项的一种特殊文件 |
| **内容** | 文字、图片、程序二进制等数据 | filename → inode 的映射关系 |
| **读写** | 可以直接被应用程序打开并读写数据 | 不能直接写入用户数据，修改目录必须通过文件系统调用 |

### inode

**inode（index node, 索引节点）**：是类 Unix 文件中用来描述一个文件元数据和磁盘位置的结构体，用来唯一标识文件系统内的对象，从而实现文件系统对文件的管理

- number：在文件系统唯一的 inode 编号
- type：普通文件、目录、符号链接、套接字
- mode：权限，三位一组，r 可读，w 可写，x 可执行，- 表示不可
- UID：所属的用户
- GID：所属的用户组
- count：链接数，即有多少目录指向该 inode
- size：以字节为单位的大小
- blocks：实际分配的磁盘块数
- atime：最后访问时间
- mtime：最后内容修改时间
...
> inode 不存文件名信息，文件名信息在目录的内容中存储

### 链接

链接（Link）：是现有文件的**额外访问路径或名字**，它不会复制文件的数据块，而是让同一个文件可以通过多个名字或路径被访问

| **对比** | **硬链接** | **软链接 / 符号链接** |
| --- | --- | --- |
| **指向目标** | 直接指向目标文件的 inode | 是一个独立文件，内容保存目标路径 |
| **占用空间** | 作为目录的一个项，不占用额外数据块 | 占用独立 inode 和数据块存路径 |
| **链接对象** | 只能指向文件，不能指向目录 | 可指向文件或目录 |
| **删除影响** | 删除任意硬链接不影响其他硬链接 | 删除原文件会导致软链接悬空失效 |
| **跨文件系统** | ❌ | ✅ |

### 磁盘调度算法

磁盘调度算法是对 I/O 请求的服务顺序，之所以需要算法，是因为磁盘存在访问延迟，需要**合理安排磁头移动顺序，减少磁头总移动距离，从而缩短平均寻道时间**

- 寻道时间：最慢，磁头移动到目标磁道所需时间
- 旋转延迟：适中，等待目标扇区转到磁头下的时间
- 传输时间：最快，实际读写数据的时间

<img src="./attachments/image/20251009_1116268.png" alt="image-20251009111628171" style="zoom:50%;" />

| **磁盘调度算法** | **选择** | **优点** | **缺点** |
| --- | --- | --- | --- |
| **FCFS, First Come First Serve** | 最先进入队列的 | 最简单 | 平均寻道时间长 |
| **SSTF, Short Seek Time First** | 距离磁头最近的 | 平均寻道最短 | 饥饿：远处请求可能长期得不到服务 |
| **SCAN** | 磁头从磁盘的一端向另一端移动，到达另一端后反向移动，移动过程中选择扫描到的请求 | 平衡效率与公平性 | 边缘磁道请求等待时间较长 |
| **C-SCAN** | 磁头从磁盘的一端向另一端移动，到达另一端后回到起点重新开始，移动过程中选择扫描到的请求 | 平均响应时间更稳定 | 回程阶段空转 |
| **LOOK / CLOOK** | 类似 SCAN / CSCAN，但只移动到当前最远请求就返回 | 减少无效移动，提高效率 | 基于当前信息，新请求可能等待较久 |